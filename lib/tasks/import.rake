...
namespace :import do
    ...
    task enrollment_data: :environment do
        
        if STDIN.tty?
            abort("Pipe in CSV to perform a data import.")
        else
            ...
            # - Import all data
            csv_text = STDIN.read() #read CSV file from stdin
            csv_text.gsub!("\xEF\xBB\xBF".force_encoding("UTF-8"), '') #remove invalid unicode BOM
            csv = CSV.parse(csv_text, :headers => true) #parse CSV with column headers
